...
\\- \\\*\\\*Definition\\\*\\\*: Hooks are functions that enable
functional
...
\\- \\\*\\\*Purpose\\\*\\\*: They simplify the way developers can use
state and
...
\\- \\\*\\\*Purpose\\\*\\\*: The \\\`useState\\\` hook allows you to add
state to

functional components .

\\- \\\*\\\*State Definition\\\*\\\*: State refers to the values or
variables
...
\\- \\\*\\\*Updating State\\\*\\\*: Use the spread operator to maintain
previous
...
\\- \\\*\\\*Purpose\\\*\\\*: The \\\`useEffect\\\` hook is used to
handle side
...
\\- \\\*\\\*Usage\\\*\\\*: It accepts two arguments: a callback function
for the
...
\\- \\\*\\\*With Empty Array\\\*\\\*: Runs only on the first render,
useful for
...
\\- \\\*\\\*Cleanup Function\\\*\\\*: Use a return function within
\\\`useEffect\\\`
...
\\- \\\*\\\*Purpose\\\*\\\*: \\\`useContext\\\` is used for managing
global data in a
...
return \<div style={{ backgroundColor: theme.background }}\>Hello
World\</div\>;
...
1\\. \\\*\\\*Create Context\\\*\\\*: Use \\\`createContext\\\` to create
a context .

2\\. \\\*\\\*Provide Context\\\*\\\*: Use the context provider to wrap
components

that need access to the context .

3\\. \\\*\\\*Consume Context\\\*\\\*: Use \\\`useContext\\\` to access
the context
...
\\- \\\*\\\*Purpose\\\*\\\*: \\\`useRef\\\` allows access to DOM
...
\\- \\\*\\\*Hooks\\\*\\\*: Functions that enable state and lifecycle
management
...
\\- \\\*\\\*useContext\\\*\\\*: Manages global state without prop
drilling.

\\- \\\*\\\*useRef\\\*\\\*: Accesses DOM elements and creates mutable
variables.
...
\\- \\\*\\\*Purpose\\\*\\\*: Allows the creation of mutable variables
that do not
...
\\\`inputElement.current\\\` to access

or update the DOM element\\\'s properties .

\\\*\\\*2. useReducer Hook\\\*\\\*

\\- \\\*\\\*Purpose\\\*\\\*: Manages state in React applications,
functioning as
...
\\- \\\*\\\*Example\\\*\\\*: A counter application using
...
\\- \\\*\\\*Purpose\\\*\\\*: Similar to \\\`useEffect\\\`, but runs
synchronously
...
\\- \\\*\\\*Difference from useEffect\\\*\\\*: \\\`useLayoutEffect\\\`
runs before

the DOM is updated, while \\\`useEffect\\\` runs after .

\\- \\\*\\\*Use Case\\\*\\\*: Measuring dimensions of an element and
applying
...
\\- \\\*\\\*Purpose\\\*\\\*: Optimizes performance by memoizing
expensive
...
\\- \\\*\\\*Example\\\*\\\*: Memoizing a calculation based on an input
number to
...
\\- \\\*\\\*Purpose\\\*\\\*: Returns a memoized callback function,
preventing
...
\\- \\\*\\\*Example\\\*\\\*: Using \\\`useCallback\\\` to optimize a
function that
...
\\- \\\*\\\*Definition\\\*\\\*: Reusable functions that encapsulate
logic using
...
{loading ? \<p\>Loading\...\</p\> : \<pre\>{JSON.stringify(data, null,
2)}\</pre\>}
